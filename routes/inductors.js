...
const { getInductor, createInductor, updateInductor, getPins, createPin, updatePin, 
...
router.get('/edit/:id', async function(req, res) {
  const inductor_id = req.params.id;
  const component_type_id = 8;

  const data = await getInductor(inductor_id);
  const pins = await getPins(inductor_id);
  const aliases = await getAliases(inductor_id);
  ...
  res.render('inductor/edit', {title: 'Edit Inductor Definition', data: data, package_types: package_types, component_sub_types: component_sub_types});
})

/* GET new inductor entry page */
router.get('/new', async function(req, res) {
 ...
 res.render('inductor/new', {title: 'New Inductor Definition', data: data, package_types: package_types, component_sub_types: component_sub_types});
});
...
router.post('/new', async function(req, res) {
  ...
  if (descr[req.body.pin_count-1]) {
    const inductor = await createInductor(data.chip_number, data.pin_count, data.package_type_id, data.component_sub_type_id, data.datasheet, data.description);
    var inductor_id = inductor.component_id;

    for (i = 0; i < req.body.pin_count; i++) {
      await createPin(inductor_id, pin[i], sym[i], descr[i]);
    }

    var aliases = data.aliases.split(',');
    for( const alias of aliases) {
      if (alias.length > 0) {
        await createAlias(inductor_id, alias.trim());
      }
    }

    res.redirect('/inductors/'+inductor_id);
  } else {
    res.render('inductor/new', {title: 'New Inductor Definition', data: data, package_types: package_types, component_sub_types: component_sub_types});
  }
});
...
router.post('/:id', async function(req, res) {
  ...
  const inductor = await updateInductor(id, data.chip_number, data.pin_count, data.package_type_id, data.component_sub_type_id, data.datasheet, data.description);
  var inductor_id = inductor.component_id;

  for (i = 0; i < req.body.pin_count; i++) {
    await updatePin(pin_id[i], inductor_id, pin[i], sym[i], descr[i]);
  }

  await deleteAliases(inductor_id);
...
/* GET inductor detail page. */
router.get('/:id', async function(req, res) {
    const id = req.params.id;
    const inductor = await getInductor(id);
    ...
    const component_type_id = inductor.component_type_id;
    ...
    if (inductor.package == 'SIP') {
      if (inductor.pin_count > 12) {
        iswide = 'dpindiagramwide';
      }
    ...
    } else if (inductor.package == 'PLCC') {
      if (inductor.pin_count > 40) {
        iswide = 'dpindiagramwide';
      }
      i = 0;
    ...
    } else if ((inductor.package == 'QFN') || (inductor.package == 'QFP')) {
      if (inductor.pin_count > 40) {
        iswide = 'dpindiagramwide';
      }
      i = 0;
    ...
    res.render('inductor/detail', { title: inductor.chip_number + ' - ' + inductor.description, data: inductor, 
