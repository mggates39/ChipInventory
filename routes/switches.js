...
router.get('/edit/:id', async function (req, res) {
  ...
  const data = await getSwitch(switch_id);
  const pins = await getPins(switch_id);
  const aliases = await getAliases(switch_id);
  const package_types =
  ...
  res.render('switch/edit', {
    title: 'Edit Switch Definition',
    data: data,
...
router.get('/new', async function (req, res) {
  ...
  res.render('switch/new', {
    title: 'New Switch Definition',
    data: data,
    package_types: package_types,
    component_sub_types: component_sub_types,
  });
...
router.post('/new', async function (req, res) {
  ...
  if (descr[req.body.pin_count - 1]) {
    const switch_item = await createSwitch(
      data.chip_number,
      data.pin_count,
    ...
    var switch_id = switch_item.component_id;
    ...
    for (const alias of aliases) {
      if (alias.length > 0) {
        await createAlias(switch_id, alias.trim());
      }
    }
...
router.post('/:id', async function (req, res) {
  ...
  const switch_item = await updateSwitch(
    id,
    data.chip_number,
    data.pin_count,
    data.package_type_id,
...
router.get('/:id', async function (req, res) {
  ...
  res.render('switch/detail', {
    title: switch_item.chip_number + ' - ' + switch_item.description,
    data: switch_item,
    pins: fixed_pins,
