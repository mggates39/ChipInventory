...
async function createNewComponent(
  ...
  if (component_type.name == 'IC') {
    ...
    const capacitor = await createCapacitor(
      component_name,
      package_type.id,
      component_sub_type.id,
      data.description,
    ...
    const capUnit = await lookupPickListEntryByName('Capacitance', data.units);
    const capacitor = await createCapacitorNetwork(
      component_name,
      package_type.id,
      component_sub_type.id,
      data.description,
    ...
    const resUnit = await lookupPickListEntryByName('Resistance', data.units);
    const resistor = await createResistorNetwork(
      component_name,
      package_type.id,
      component_sub_type.id,
      data.description,
      ...
      data.tolerance,
      data.power,
      data.number_resistors,
      data.datasheet,
    );
    component_id = resistor.component_id;
  } else if (component_type.name == 'Diode') {
    const fvUnits = await lookupPickListEntryByName(
      'Voltages',
      data.forward_units,
    );
    const rvUnits = await lookupPickListEntryByName(
      'Voltages',
      data.reverse_units,
    );
    const lightColor = await lookupPickListEntryByName(
      'LEDColor',
      data.light_color,
    );
    const lensColor = await lookupPickListEntryByName(
      'LensColor',
      data.lens_color,
    );
    ...
    const ratingUnits = await lookupPickListEntryByName(
      'FuseRating',
      data.rating_units,
    );
    const voltages = await lookupPickListEntryByName(
      'Voltages',
      data.voltage_units,
    );
    ...
    const jack = await createConnector(
      component_type.id,
      component_name,
      data.pincount,
      package_type.id,
    ...
    const plug = await createConnector(
      component_type.id,
      component_name,
      data.pincount,
      package_type.id,
    ...
    const crystal = await createCrystal(
      component_name,
      data.frequency,
      units.id,
      data.pincount,
    ...
    const usage = await lookupPickListEntryByName(
      'TransistorUsage',
      data.usage,
    );
    const power_units = await lookupPickListEntryByName(
      'Power',
      data.power_units,
    );
    const threshold_units = await lookupPickListEntryByName(
      'Voltages',
      data.threshold_units,
    );
    const transistor = await createTransistor(
      component_name,
      data.description,
      data.pincount,
      package_type.id,
    ...
    const switch_object = await createSwitch(
      component_name,
      data.pincount,
      package_type.id,
      component_sub_type.id,
...
async function import_component(name, data) {
  ...
  if (data.subtype) {
    component_sub_type = await lookupComponentSubType(
      component_type_id,
      data.subtype,
    );
  } else if (data.family) {
  ...
  if (components[0]) {
    ...
    component_id = await createNewComponent(
      chip_number,
      data,
      package_type,
      component_type,
